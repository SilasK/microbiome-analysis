...
for (comparison in names(Comparisons))
{
  ...
  d.eff <- aldex.effect(x, conditions,useMC = T,include.sample.summary = T)
  
  if (! all(sort(Comparisons[[comparison]]) == Comparisons[[comparison]]))
  {
    # alphanumeric sorting will inverse comparison
    
    d.eff$effect= - d.eff$effect
    d.eff$diff.btw= - d.eff$diff.btw
